...
(() => {
  ...
  function initStickmanMode() {
    ...
    function spawnLandImpact(x, y) {
      const el = document.createElement("div");
      el.className = "land-impact";
      el.style.cssText = "position:fixed;left:" + x + "px;top:" + y + "px;pointer-events:none;z-index:16;";
    ...
    function runWithTrampoline(stickman, targetBtn) {
      if (busy) return;
      busy = true;

      // Capture button position NOW (content still visible)
      const btnRect = targetBtn.getBoundingClientRect();
      const landX   = btnRect.left + btnRect.width / 2;
      // div center = hip level (65px from div top, div is 130px)
      // hip должен быть на верхнем крае кнопки → top = btnRect.top
      const landY   = btnRect.top;

      const sRect   = stickman.getBoundingClientRect();
      const startX  = sRect.left + sRect.width / 2;
      const centerX = window.innerWidth / 2;
      const bottomY = window.innerHeight - 55;

      // Stop idle animation, snap to fixed ground — content stays visible
      stickman.style.animation  = "none";
      stickman.style.transition = "none";
      stickman.style.position   = "fixed";
      stickman.style.left       = startX + "px";
  ...
  (function initSleep() {
    ...
    function wakeUp() {
      document.querySelectorAll(".stickman.sleepy").forEach(s => {
        s.classList.remove("sleepy");
